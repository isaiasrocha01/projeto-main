 const meses = ["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"];
    ...
    function dataCasamentoExtenso(dataStr) {
      if (!dataStr) return "____";
      const partes = dataStr.split('-');
      const d = new Date(Number(partes[0]), Number(partes[1]) - 1, Number(partes[2]));
  ...
  ano = Number(ano);
  if (ano < 1000) return numeroPorExtenso(ano);

  const mil = Math.floor(ano / 1000);
  const resto = ano % 1000;

  let milExt = (mil === 1) ? "mil" : numeroPorExtenso(mil) + " mil";

  if (resto === 0) return milExt;
  if (resto < 100) return milExt + " e " + numeroPorExtenso(resto);
  return milExt + " " + numeroPorExtenso(resto);
}

function dataParaEditalOuDiario(dataStr) {
  if (!dataStr) return "____";
  const partes = dataStr.split('-');
  const d = new Date(Number(partes[0]), Number(partes[1]) - 1, Number(partes[2]));
  ...
  const diaExt = numeroPorExtenso(d.getDate());
  const mesExt = meses[d.getMonth()];
  const anoExt = anoPorExtenso(ano);

  const edital = `${diaExt} DE ${mesExt.toUpperCase()} DE ${anoExt.toUpperCase()} (${dia}/${mes}/${ano})`;
  const diario = `PUBLICADO NO DIÁRIO OFICIAL EM ${edital}`;

  return { edital, diario };
}
    ...
    document.getElementById('gerar').addEventListener('click', function(e){
      ...
      const dataCasamento = dataCasamentoExtenso(document.getElementById('dataCasamento').value);
      ...
      const datasEditalDiario = dataParaEditalOuDiario(document.getElementById('dataTermo').value);
      const dataTermoEdital = datasEditalDiario.edital;
      const dataTermoDiario = datasEditalDiario.diario;




      const texto = `Aos ${dataTermo}, neste ofício, ____________________, nos termos da Lei nº 1110 de 23 de maio de 1950, faço a inscrição do casamento religioso de ${nomeCônjuge1}, que passou a adotar o nome de ${nomeAtualCônjuge1} e ${nomeCônjuge2}, que passou a adotar o nome de ${nomeAtualCônjuge2}, comigo ______________________ comarca de SALVADOR-BA, celebrado ${localdaCelebracao}, no dia ${dataCasamento}, pelo(a) ${celebrante}, observada a prescrição do artigo 1.534 do Código Civil conforme prova nos autos existentes no Cartório. ${genero1} ${nomeCônjuge1}, de CPF: ${cpf1}, ${civil1}, ${profissao1}, NASCIDO ${natural1}, de NACIONALIDADE ${nacionalidade1}, EM ${nascimento1Ext}, residente e domiciliado ${endereco1}, Filho(a) de ${filiacao1}. ${genero2}  ${nomeCônjuge2}, de CPF: ${cpf2}, ${civil2}, ${profissao2}, NASCIDO ${natural2}, de NACIONALIDADE ${nacionalidade2}, EM ${nascimento2Ext}, residente e domiciliado ${endereco2}, Filho(a) de ${filiacao2}. Foram testemunhas: ${testemunhas}. O regime de bens adotado é o da ${RegimedeBens},  Afixado o Edital de Proclamas em ${dataTermoEdital}, e publicado pelo Diário do Poder Judiciário em ${dataTermoDiario}. Exibiram os documentos exigidos pelo artigo 1.525 do Código Civil Brasileiro sob os números la V, a saber memorial com as declarações de ambos e seus respectivos pais. " Nada mais contém o dito termo donde fielmente foi extraída a presente certidão, depois de lhes ser lido por mim: _________________________, que subscrevi e dou fé.`;
